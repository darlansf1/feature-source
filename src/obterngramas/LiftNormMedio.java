 ...
 * @author ragero
 */
public class LiftNormMedio {

    public LiftNormMedio(ArrayList<File> filesIn, ArrayList<File> filesOut, Float supMin, boolean repetir, Float minLift, int rank){
        
        ArrayList<String> itemsets;
        HashMap<String,Float> hashSup;
        HashMap<String,Float> hashConf;
        HashMap<String,Float> hashLift;
        HashMap<String,Float> hashLift2;
        double rangeMax = 0, rangeMin = 0, range = 0, limDep = 0;
        float supMinOrig = supMin;
        ...
        for(int i=0;i<filesIn.size();i++){
            ...
            hashLift2 = new HashMap<String,Float>();
            float minSupMin = 300000;
            supMin = supMinOrig;
            ...
            try{
                ...
                rangeMin = (4*minSupMin)/(Math.pow((1 + minSupMin), 2));
                rangeMax = (1/minSupMin);
                ...
                minLift = (float)media/(float)hashLift2.keySet().size();
                System.out.println("Kappa Medio: " + minLift + "--------------------------------------------------");
                ...
                if(rank > 0){
                    ...
                    for(int k=0; k<items.length; k++){
                        ...
                        if(!itemset.contains(" ")){
                            ...
                            ItemsetMed rule = new ItemsetMed(itemset, (double)hashLift2.get(itemset));
                            sortRules.add(rule);
                        }
                    }
                    ...
                    if(repetir == true){
                        for(int j=0;j<itemsets.size();j++){
                            String linha = itemsets.get(j);
                            Float valor = hashLift2.get(linha);
