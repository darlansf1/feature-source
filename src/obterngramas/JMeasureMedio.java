 ...
 * @author ragero
 */
public class JMeasureMedio {

    public JMeasureMedio(ArrayList<File> filesIn, ArrayList<File> filesOut, Float supMin, boolean repetir, Float jMeasureMin, int rank){
        
        ArrayList<String> itemsets;
        HashMap<String,Float> hashSup;
        HashMap<String,Double> hashJMeasure;
        ...
        for(int i=0;i<filesIn.size();i++){
            ...
            hashJMeasure = new HashMap<String,Double>();
            ...
            try{
                ...
                while((line = fileRules.readLine())!=null){
                    ...
                    if(suporte >= supMin){
                        Double jMeasure = 0.0;
                        try{
                            double jMeasure1 = suporte * (Math.log10(suporte/(hashSup.get(cabeca) * hashSup.get(corpo)))/0.301);
                            double jMeasure2 = (hashSup.get(cabeca) - suporte) * (Math.log10((hashSup.get(cabeca) - suporte)/(hashSup.get(cabeca) * (1 - hashSup.get(corpo))))/0.301);
                            jMeasure = jMeasure1 + jMeasure2;
                        }catch(Exception e){
                        ...
                        if(!itemsets.contains(linha)){
                            ...
                            hashJMeasure.put(linha, jMeasure);
                        }else{ 
                            Double valor = hashJMeasure.get(linha);
                            if(jMeasure>valor){
                                hashJMeasure.remove(linha);
                                hashJMeasure.put(linha, jMeasure);
                            }
                        }
                    }    
                ...
                double jMeasureMed =0;
                float kitemsets =0;
                for(int j=0;j<itemsets.size();j++){
                    String linha = itemsets.get(j);
                    if(linha.contains(" ")){
                        Double valJMeasure = hashJMeasure.get(linha);
                        jMeasureMed = jMeasureMed + valJMeasure;
                        kitemsets++;
                        //System.out.println("Itemset: " + linha + "  Confianca: " + confRegra);
                    }
                }
                
                System.out.println("Total: " + jMeasureMed);
                jMeasureMed = (float)jMeasureMed/(float)kitemsets;            
                System.out.println("J-Measure Media: " + jMeasureMed + "--------------------------------------------------");
                ...
                if(rank > 0){
                    ...
                    Object[] items = hashJMeasure.keySet().toArray();
                    for(int k=0; k<items.length; k++){
                        ...
                        if(!itemset.contains(" ")){
                            ...
                            ItemsetMed rule = new ItemsetMed(itemset, hashJMeasure.get(itemset));
                            sortRules.add(rule);
                        }
                    }
